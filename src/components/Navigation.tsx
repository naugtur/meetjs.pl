...
export const Navigation = () => {
	return (
		<>
			...
			<Disclosure as="nav" className="sticky top-0 z-20 bg-purple" aria-label="Main navigation" role="navigation">
				{({ open }) => (
					<>
						...
						<div className="relative flex h-16 items-center justify-between">
							<div className="flex flex-1 items-center justify-between sm:items-stretch">
								<Logo />
								{/* Desktop navigation */}
								<nav aria-label="Main" className="hidden sm:flex" itemScope itemType="https://schema.org/SiteNavigationElement">
										...
										{menuLinks.map((item) => (
											<li key={item.name}>
												<Link
													...
													{...(item.external && { 'aria-label': `${item.name} (opens in a new tab)` })}
												>
													{item.name}
													{item.external && (
					...
					<DisclosurePanel className="sm:hidden">
						{/* Mobile navigation */}
						<nav aria-label="Main mobile" itemScope itemType="https://schema.org/SiteNavigationElement">
