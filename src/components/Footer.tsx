		...
		<footer className="bg-primary text-white">
			<div className="mx-auto flex max-w-7xl flex-col justify-between px-2 sm:px-6 md:flex-row lg:px-8">
				<section className="flex flex-col">
					...
					<ul className="flex flex-col items-start justify-center gap-4 px-4 text-xl md:gap-2">
						{instagramLinksData.map((socialLink) => (
							<li key={socialLink.name}>
								<Link href={socialLink.url} target="_blank" className="flex items-center justify-center gap-2">
									{socialLink.icon}
						...
						<li>
							<Link href="mailto:contact@meetjs.pl" className="flex items-center justify-center gap-2">
								<FaRegEnvelope />
								contact@meetjs.pl
							</Link>
						</li>
					...
					<p className="p-4">© {new Date().getFullYear()} meet.js. All rights reserved.</p>
				</section>
